...
if (!version) {
  console.error('❌ Please specify a version. Example: node scripts/bump-version.js 0.3.7');
  process.exit(1);
}

// Validate version format (basic semver check)
if (!/^\d+\.\d+\.\d+(-[a-zA-Z0-9.]+)?$/.test(version)) {
  console.error(`❌ Invalid version format: "${version}". Expected format: X.Y.Z (e.g. 0.3.7)`);
  process.exit(1);
}
