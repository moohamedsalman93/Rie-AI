...
class Desktop:
    ...
    def bring_window_to_top(self,target_handle:int):
        ...
        # If no foreground window, we don't need to attach input
        if not foreground_handle or foreground_handle == target_handle:
            try:
        ...
        # Only attach if threads are different and valid
        should_detach = False
        try:
            ...
            if should_detach:
                try:
                    ...
                    # Ignore cleanup errors to prevent stream crash
                    pass
